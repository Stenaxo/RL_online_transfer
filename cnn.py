...
def main_worker(args):
    ...
    for r in range(args.repetition):
        ...
        if min(list_val_loss) < best_loss:
            best_loss = min(list_val_loss)
            torch.save(
                model.state_dict(), os.path.join("result", "best_model_loss.pth")
            )
...
def train(modele, optimizer, criterion, device, trainloader, n_epoch, valloader, sched):
    """
