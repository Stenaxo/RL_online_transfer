...
def main():
    ...
    if not os.path.exists('result_cnn'):
        os.makedirs('result_cnn')
...
def iterate_data(dataloader, model, criterion, device, optimizer=None, is_training=False, scheduler = None):
...
def train(modele, optimizer, criterion, device, trainloader, n_epoch, valloader, condition, repeat, sched):
...
# save the standard deviations to csv
    ...
    plot_graph(name = "Average training accuracy of the CNN model",
               object = averages_train_accuracy, 
               label_one="train",
               name_f='train_accuracy',
               std_dev=std_train_accuracy)
    plot_graph(name = "Average training loss of the CNN model",
               object = averages_train_loss, 
               label_one="train",
               name_f='train_loss',
               std_dev= std_train_loss)
    plot_graph(name = "Average validation loss of the CNN model",
               object=averages_val_loss,
               label_one="val",
               name_f='val_loss',
               std_dev= std_val_loss)
    plot_graph(name= "Average validation accuracy of the CNN model",
    ...
    torch.save(modele.state_dict(), os.path.join('result_cnn','parameters.pth'))
"""        
...
def plot_graph(name, name_f, object, label_one, std_dev=None, ylabel_loss = True):
    plt.style.use("ggplot")
    plt.rcParams.update()
    plt.figure(figsize=(10, 5))
    plt.title(name)
    ...
    if ylabel_loss == True:
        ...
        plt.ylabel("accuracy")
    plt.legend()
    plt.savefig(os.path.join('result_cnn',f'{name_f}.png'))
...
def save_results_to_csv(filename, train_loss, train_accuracy, val_loss, val_accuracy):
    ...
    with open(os.path.join('result_cnn', filename), 'w', newline='') as file:
...
def plot_rep(df, title, filename, ylabel, num_repetitions):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join('result_cnn',f'{filename}.png'))
